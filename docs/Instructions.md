...
道路と建物はプロパティに EDGES <mark>という項目</mark>があり，これはエッジ（線）の開始位置と終了位置がリストとして格納されています．
これらを1つ1つ結びつけることによって，建物と道路の描画をおこなっています．

### BLOCKADE

瓦礫はプロパティに APEXES  <mark>という項目</mark>があり，これは apexes（頂点）の x と y が交互にリストとして格納されています．
...
注意点として，この線はエンティティが移動する経路を示しているものではなく，移動に対して通過する建物や道路を示しているものです．
よって，ここで示されている線は通過する建物の重心を結んだ線です．
そのため，青色の線が道路や建物からはみ出して描画される場合もあります．
また，この道路や建物のエンティティID が格納されているリストの最初の ID はそのエンティティが移動を開始した場所の道路や建物のエンティティ ID も含まれています．
...
<mark>
AK_CLEAR_AREA は瓦礫の啓開をおこなう地点までの x と y 座標が含まれています．
COMMAND_CLEAR_AREA レイヤーは，その土木隊の位置から，その土木隊が啓開をおこなう地点までの x と y 座標までを青色の線で結んで描画しています．<br>
...
また，ここでの情報は現在見ている情報だけではなく，過去に認知した情報も全て描画されます．
...
<mark>
名称がperceptionで始まるレイヤーには，「view perception」にて選択されたエンティティが実際に見た情報が描画されています．
知覚情報を参照しているエンティティは，水色のマークで表示されます．
また，ここには現在見ている情報だけではなく，過去に認知した情報も全て描画されます．
よって，これらのレイヤーには古い情報と新しい情報が含まれます．
また，現在のステップにおけるエージェントの行動は，1ステップ前の知覚情報によるものであると言えます．
よって，perceptionレイヤーで描画される内容は，そのエージェントが行動をする1ステップ前に見た知覚情報になります．<br>
また，全エージェントは0ステップ時にマップの情報だけ渡されるため，知覚していない建物や道路などは0ステップにある素の状態を描画しています．
</mark>
...
<mark>communicationから始まるレイヤーには，知覚情報を見ているエージェントが受信した通信が描画されています．
</mark>
...
通信でやり取りされるのは，エージェント自らの知覚情報と，他のエージェントから受信した知覚情報になります．
そのため，自身の発信を受信した他のエージェントにおいて，そのエージェントが発信した通信を自身が受け取った場合，自身が発信した知覚情報を再度受信することになります．
また，通信の送受信にはステップが複数回必要になります．
その結果として，描画される受信した情報に過去の自分の情報が含まれている場合水色のエージェントが2体表示される場合があります．
</mark>
...
アークの色は，開始点については各エージェントと同じ色で，到着点に近づくにつれて黄色に変化していきます．
